  ...
  for (const [agent, given] of Object.entries(policy)) {
    ...
    for (const rule of rules) {
      ...
      for (const permission of PERMISSIONS) {
        if (rule[permission] !== undefined && prior?.[permission] !== undefined) {
          bad(`two rules for ${JSON.stringify(rule.prefix)} both specify ${permission}`);
        }
      }
    ...
    for (const { prefix } of rules) {
      if (resolve(rules, "write", prefix) && !resolve(rules, "read", prefix)) {
        bad(
          `${JSON.stringify(prefix)} is writable but not readable — an agent that cannot ` +
 ...
 * escaped out of a namespace name.
 */
function readSql(rules: ScopeRule[]): Scope["readSql"] {
  ...
  const whens = rules
    ...
    .map((r) => {
      params.push(r.prefix.length, r.prefix);
      return `when substr(n.path, 1, ?) = ? then ${r.read ? 1 : 0}`;
    });
